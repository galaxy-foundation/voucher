...
function Nav() {
    ...
    useEffect(() => {
        if (wallet.status === "error") {
            alert("Plase change chain to ethereum Chain");
        }
    }, [wallet.status]);
    ...
    useEffect(() => {
        if (window.ethereum !== undefined) checkConnection();
    }, [chainId]);
    ...
    return (
        <div className="x-navContainer">
            ...
            <div className="x-nav-buttons">
                {/* <Link to="/" className = "x-nav-connect"><button className = "x-nav-connect" >Buy</button></Link>
			<Link to="/Buy" className = "x-nav-connect"><button className = "x-nav-connect" >BuyWithQRCode</button></Link> */}
